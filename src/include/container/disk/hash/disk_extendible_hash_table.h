...
class DiskExtendibleHashTable {
 ...
 private:
  ...
  void UpdateDirectoryMapping(ExtendibleHTableDirectoryPage *directory, uint32_t new_bucket_idx,
                              page_id_t new_bucket_page_id, uint32_t new_local_depth, uint32_t diff);
  ...
  void MergeBucket(ExtendibleHTableDirectoryPage *directory_page, ExtendibleHTableBucketPage<K, V, KC> *bucket_page,
                   uint32_t bucket_id);
